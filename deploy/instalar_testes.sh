    ...
    case "$PROD_DATABASE_URL" in
        ...
        postgres*)
            ...
            if [ -n "$PSQL_ADMIN" ]; then
                ...
                # Este instalador NÃO altera o pg_hba.conf nem qualquer outra
                # configuração do Postgres: regra de autenticação é do DBA.
                HBA="$($PSQL_ADMIN "psql -tAc 'show hba_file'" 2>/dev/null | tr -d '[:space:]')"
                if [ -n "$HBA" ] && [ -f "$HBA" ] \
                   && ! grep -Eq "^\s*(local|host|hostssl|hostnossl)\s+[^#]*\b$PG_TESTE\b" "$HBA"; then
                    echo "   AVISO: $HBA não tem regra para $PG_TESTE. Se o restore ou o portal"
                    echo "          falharem por autenticação, peça ao DBA uma regra equivalente à de $PG_NOME."
                fi
            else
