...
3.  Ela FAZ coisas no device — tag e exclusão? (isso é a API do Workspace
    ONE UEM, que é outra API, outro host e outro escopo)
...
def main() -> int:
    ...
    print("\n[2] Workspace ONE UEM — TAG e EXCLUSÃO")
    if not a.uem:
        ...
        print("\n  Se as leituras acima responderem 200, as ESCRITAS existem no mesmo")
        print("  host e seguem o padrão (NÃO foram disparadas por este script):")
        print("    incluir tag : POST /API/mdm/tags/{tagId}/adddevices")
        print("    remover tag : POST /API/mdm/tags/{tagId}/removedevices")
        print("    excluir     : DELETE /API/mdm/devices/{id}")
        print("  Confirme cada uma na documentação da API da UEM antes de automatizar:")
        print("  excluir device é irreversível e não tem desfazer.")
