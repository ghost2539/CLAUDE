"""CAPEX Spare — projetos e itens de investimento do SPARE. API /api/capex-spare.
...
Banco próprio (`db/capex_spare.py`). Permissão pelo módulo
`capex_spare`: view lê, create inclui, edit altera/exclui, admin idem.

Não confundir com "Orçamento Spare" (`routers/orcamento_spare_exec.py`), que
é a tela do Controle de Orçamento servida para a área — outro produto, outra
permissão.
"""
...
_log = logging.getLogger("capex_spare")
MODULO = "capex_spare"

router = APIRouter(prefix="/api/capex-spare", tags=["CAPEX Spare"],
